...
#define TAG "com.oodles.smslistener"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> SmsListener_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("SmsListener.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::SmsListenerBindings::lookupGeneratedInit(
...
static void SmsListener_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(SmsListener_getBinding)->GetFunction());
}

static void SmsListener_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::SmsListenerBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry SmsListenerBinding = {
...
Java_com_oodles_smslistener_SmsListenerBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.oodles.smslistener", &SmsListenerBinding);
	titanium::KrollBindings::addExternalLookup(&(::SmsListenerBindings::lookupGeneratedInit));
}
